// Package main 是 demo 子应用 A(端口 5001)。
package main
...
const (
	...
	callback  = "http://localhost:5001/callback"
)
...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	sess := getSession(r)
	if sess == nil {
		fmt.Fprintf(w, `<h1>%s</h1><a href="/login">SSO 登录</a>`, appName)
		return
	}
	fmt.Fprintf(w, `<h1>已登录 %s</h1><p>用户:%s</p><a href="/logout">退出</a>`, appName, sess.Username)
}
